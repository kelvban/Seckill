...
//                        }
    ...
    @RequestMapping("/test-dead")
    @ResponseBody
    public Result<String> testDead(@RequestParam Integer size,
                                     @RequestParam Integer range,
                                     @RequestParam String batch){
        Random random=new Random();
        List<TestMessage> list=new ArrayList<>();
        ...
        executorService.execute(new Runnable() {
            ...
            public void run() {
                ...
                if(CollectionUtils.isNotEmpty(list)){
                    for (TestMessage testMessage:list){
                        mqSender.sendDeadMessage(testMessage);
                    }
                }
                Long endTime=System.currentTimeMillis();
                System.out.println("推送总用时："+(endTime-startTime));
    ...
    @RequestMapping("/test-delay-ab")
    @ResponseBody
    public Result<String> testDelayAB(@RequestParam Integer size,
                                   @RequestParam Integer range,
                                   @RequestParam String batch){
        Random random=new Random();
        List<TestMessage> list=new ArrayList<>();
        ...
        executorService.execute(new Runnable() {
            ...
            public void run() {
                ...
                if(CollectionUtils.isNotEmpty(list)){
                    for(int i=0;i<list.size();i++){
                        if(i%2==0){
                            mqSender.sendDelayABMessage(list.get(i),"delay.qa.msg",new Long(10000));
                        }else{
                            mqSender.sendDelayABMessage(list.get(i),"delay.qb.msg",new Long(100000));
                        }
                    }
                }
                Long endTime=System.currentTimeMillis();
    ...
    @RequestMapping("/test-delay-plugin")
    @ResponseBody
    public Result<String> testDelayPlugin(@RequestParam Integer size,
                                      @RequestParam Integer range,
                                      @RequestParam String batch){
        Random random=new Random();
        List<TestMessage> list=new ArrayList<>();
        ...
        executorService.execute(new Runnable() {
            ...
            public void run() {
                ...
                if(CollectionUtils.isNotEmpty(list)){
                    for(int i=0;i<list.size();i++){
                        mqSender.sendDelayPluginMessage(list.get(i),"delay.plugin.msg",new Long(100000));
                    }
                }
                Long endTime=System.currentTimeMillis();
                System.out.println("推送总用时："+(endTime-startTime));
